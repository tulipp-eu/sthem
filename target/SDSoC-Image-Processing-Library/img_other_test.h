...
void swTestColorConversion(void) {
	...
	for (vx_uint32 k = 0; k < 2; k++) {
		...
		if (k == 0) {
			...
			imgConvertColor<vx_uint8, vx_uint32, MATRIX_LENGHT_A, MATRIX_LENGHT_A, VX_DF_IMAGE_U8, VX_DF_IMAGE_RGBX>(src_a, dst_a);
		}
		if (k == 1) {
			printf("Color Conversion Grayscale to RGB:\n");
			imgConvertColor<vx_uint8, vx_uint32, MATRIX_LENGHT_A, MATRIX_LENGHT_A, VX_DF_IMAGE_U8, VX_DF_IMAGE_RGB>(src_a, dst_a);
		}
	...
	for (vx_uint32 k = 0; k < 2; k++) {
		...
		if (k == 0) {
			...
			imgConvertColor<vx_uint32, vx_uint8, MATRIX_LENGHT_B, MATRIX_LENGHT_B, VX_DF_IMAGE_RGBX, VX_DF_IMAGE_U8>(src_b, dst_b);
		}
		if (k == 1) {
			printf("Color Conversion RGB to Grayscale:\n");
			imgConvertColor<vx_uint32, vx_uint8, MATRIX_LENGHT_B, MATRIX_LENGHT_B, VX_DF_IMAGE_RGB, VX_DF_IMAGE_U8>(src_b, dst_b);
		}
...
void swTestScaleDown(void) {
	...
	for (vx_uint32 k = 0; k < 2; k++) {
		...
		if (k == 0) {
			...
			imgScaleDown<SRC_SIZE, SRC_SIZE, DST_SIZE, DST_SIZE, VX_INTERPOLATION_NEAREST_NEIGHBOR>(in_array, out_array);
		}
		if (k == 1) {
			printf("Scale image using Bilinear Interpolation:\n");
			imgScaleDown<SRC_SIZE, SRC_SIZE, DST_SIZE, DST_SIZE, VX_INTERPOLATION_BILINEAR>(in_array, out_array);
		}
