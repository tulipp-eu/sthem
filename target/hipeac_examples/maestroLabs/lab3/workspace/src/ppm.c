...
void ppmToStream(u32* stream, ppmImage* image)
{
    ...
    for (u32 y = 0; y < height; y++) {
        for (u32 x = 0; x < width; x++) {
            stream[y * width + x] =
                ((u32) image->buffer[y * width + x].r << 24)
                | ((u32) image->buffer[y * width + x].g << 16)
                | ((u32) image->buffer[y * width + x].b << 8);
        }
    }
}
...
void streamToPpm(u32* stream, ppmImage* image)
{
    ...
    for (u32 y = 0; y < height; y++) {
        for (u32 x = 0; x < width; x++) {
            image->buffer[y * width + x].r =
                (u8)(0xFF & (stream[y * width + x] >> 24));
            image->buffer[y * width + x].g =
                (u8)(0xFF & (stream[y * width + x] >> 16));
            image->buffer[y * width + x].b =
                (u8)(0xFF & (stream[y * width + x] >> 8));
        }
    }
}

int readPpmToStream(u32* stream,
                    char const* filename,
...
int writeStreamToPpm(u32* stream,
                     char const* filename,
                     unsigned int const width,
                     unsigned int const height)
{
...
{
    ppmImage* image = (ppmImage*) malloc(sizeof(ppmImage));
    char buf[READ_BUFLEN + 1];
    memset(buf, '\0', READ_BUFLEN + 1);
    int bytesRead = 0;
    ...
    image->buffer =
        (pixel*) malloc(sizeof(pixel) * image->width * image->height);
    if (image->buffer == NULL) {
        EPRINTF("Cannot allocate image buffer!\n");
        goto ERR_BUF;
    }
...
{
    char buf[HEADER_MAX_LEN + 1];
    memset(buf, '\0', HEADER_MAX_LEN + 1);
    ...
    if (f_write(&fp, buf, lenHeader, &bytesWritten) != FR_OK
        || (unsigned int) lenHeader != bytesWritten) {
        goto ERR;
    };
    ...
    char* p = (char*) image->buffer;
    FRESULT writeStatus = f_write(&fp, p, bytesToWrite, &bytesWritten);
    if (FR_OK != writeStatus || bytesWritten != bytesToWrite) {
        f_close(&fp);
        goto ERR;
    }
...
ERR:
    EXITFAIL("Could not save ppm to file");
}
