...
extern void Fill_boot_reply_package( struct BootInitReplyPackage  * boot_reply_package) ;
...
extern int SaveHeaderToFlash(struct ResetPackage inBuffer);
...
void USB_BootReset(struct ResetPackage *inBuffer)
{
	//finalize operation
	long j;
